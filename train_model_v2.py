...
MODEL_SAVE_PATH = "voice_model_v3.pkl"
SAMPLE_RATE = 16000
...
def extract_file_features(file_path, is_training=False, file_idx=0):
    """
    Extracts 94 acoustic features purely from the decoded audio waveform array:
      - Absolutely NO metadata, filename extension, or path features are used.
      - If is_training=True, waveform augmentation is applied equally to real and fake samples.
      - Features extracted:
    ...
    try:
        ...
        # Apply augmentation ONLY during training on the decoded 1D audio array
        if is_training:
            seed = (RANDOM_SEED + file_idx * 13) % (2**31 - 1)
            y = augment_audio_waveform(y, sr=sr, seed=seed)

        # 1. MFCC (20) & Delta MFCC (20)
...
def load_dataset_split(split_name, is_training=False):
    ...
    # Parallel CPU extraction using ProcessPoolExecutor
...
def main():
    print("==================================================")
    print("  AI VOICE DETECTION - MODEL TRAINING (v3)")
    print("==================================================")
    print(f"Dataset Path: {DATASET_BASE_DIR}")
    print(f"Random Seed:  {RANDOM_SEED}")
    print(f"Output Model: {MODEL_SAVE_PATH}")

    # 1. Load Training Data WITH Augmentation (applied equally to real & fake samples)
    ...
    # 5. Save Newly Trained Model as voice_model_v3.pkl
    joblib.dump(model, MODEL_SAVE_PATH)
    print(f"Saved newly trained model to: {MODEL_SAVE_PATH}")
    print("Original 'voice_model.pkl' and 'voice_model_v2.pkl' remain unchanged.")
